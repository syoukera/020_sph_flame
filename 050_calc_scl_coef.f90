subroutine calc_scl_coef(phi, G_i, S_i, a_i, b_i, c_i, d_i,n_up)
...
!   ------- n=1 center boundary condition ---------
    Gamma_e  = xvel(1)/xscl(2)*(G_i(2)-G_i(1))+G_i(1)
...
!
        ...
        Gamma_e  = (delt_xc_e/delt_x_e)*(G_i(n+1)-G_i(n))+G_i(n)
        Gamma_w  = (delt_xc_w/delt_x_w)*(G_i(n)-G_i(n-1))+G_i(n-1)
